...
const uploadExplanationFile = async () => {
  ...
  try {
    const response = await accommodationServices.uploadExplanationFile(
      newAccomID.value,
      currentFile.value
    );
...
async function save() {
  ...
  if (errorMessage) router.push({ name: "adminViewAccom" });
}
...
<template>
  <v-row class="pa-4 text-h5 font-weight-bold"> Add Accommodation </v-row>
  ...
  <div class="pa-4">
    <v-card style="background-color: #d5dfe7" class="pa-4">
      ...
      <div>
        <p class="pl-5" style="font-weight: bold">
          Accommodation Explanation (Optional)
        </p>
        <v-file-input
